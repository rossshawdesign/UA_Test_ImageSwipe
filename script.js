...
window.addEventListener('load', () => {
  ...
  // =====================================
  // LOAD IMAGES WITH PIXI.Texture.from()
  // =====================================
  textures = imageUrls.map(url => PIXI.Texture.from(url));

  // Check that all textures are valid before starting
  let loadedCount = 0;
  textures.forEach(tex => {
    if (tex.baseTexture.valid) loadedCount++;
    else tex.baseTexture.on("loaded", () => {
      loadedCount++;
      if (loadedCount === textures.length) startCarousel();
    });
  });
  if (loadedCount === textures.length) startCarousel();
